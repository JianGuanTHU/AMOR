...
class Indexer(object):
    ...
    def search_knn(self, query_vectors: np.array, top_docs: int, index_batch_size: int = 2048, select_ids = None) -> List[Tuple[List[object], List[float]]]:
        ...
        for k in tqdm(range(nbatch)):
            ...
            if select_ids is not None:
                id_selector = faiss.IDSelectorArray(select_ids)
                scores, indexes = self.index.search(q, top_docs, params=faiss.SearchParametersIVF(sel=id_selector))
            else:
