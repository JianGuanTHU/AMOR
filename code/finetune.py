...
def encode_with_prompt_completion_format(example, tokenizer, max_seq_length):
    ...
    expert_idx = torch.tensor([example['expert']]).type_as(attention_mask)
    return {
        'input_ids': input_ids.flatten(),
        'labels': labels.flatten(),
        'attention_mask': attention_mask.flatten(),
        'expert_idx': expert_idx.flatten()
    }
...
def main():
    ...
    with accelerator.main_process_first():
        lm_datasets = raw_datasets.map(
            ...
            remove_columns=[name for name in raw_datasets["train"].column_names if name not in ["input_ids", "labels", "attention_mask", "expert_idx"]],
