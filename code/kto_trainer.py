...
def _tokenize(
    ...
    prompt_and_completion = [prompt.strip() + " " + completion.strip() for prompt, completion in zip(batch["prompt"], batch["completion"])]
...
class KTOTrainer(Trainer):
    ...
    def get_batch_logps(
        ...
        loss = (per_token_logps * loss_mask).sum(-1)
        return loss, loss / loss_mask.sum(-1)
        # if average_log_prob:
    ...
    def forward(
        ...
        if self.is_encoder_decoder:
            ...
            expert_idx = torch.tensor(batch['expert']).type_as(batch["KL_completion_attention_mask"])
            with torch.no_grad():
                KL_logits = model(
        ...
        completion_logps, completion_avg_logps = self.get_batch_logps(
            completion_logits,
        ...
        chosen_avg_logps = completion_avg_logps[chosen_idx, ...]

        chosen_logits = completion_logits[chosen_idx, ...]
        rejected_logits = completion_logits[rejected_idx, ...]

        return (chosen_logps, rejected_logps, chosen_logits, rejected_logits, KL_logps, chosen_avg_logps)
    ...
    def kto_loss(
        ...
        policy_chosen_avg_logps: torch.FloatTensor,
    ) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        ...
        mle_lossess = -policy_chosen_avg_logps.nanmean()
        if torch.isnan(mle_lossess):
            mle_lossess = torch.zeros_like(mle_lossess)
        losses = losses.nanmean() + mle_lossess

        losses.requires_grad_(True)

        print(f"losses: {losses}, mle_loss: {mle_lossess}, chosen_losses: {chosen_losses}, rejected_losses: {rejected_losses}")
